...
if query and market:
    ...
    if candidates:
        ...
        st.divider()
        share_text = (
            f"{data['name']} の株価: {data['price']} ({data['change']:+.2f}%)\n"
            f"PER: {data['per']} / PBR: {data['pbr']} / 配当利回り: {data['dividend_yield']:.2f}%\n"
            f"#milfo で調べました"
        )
        import urllib.parse
        line_url = "https://social-plugins.line.me/lineit/share?url=" + urllib.parse.quote(
            "https://milfo.onrender.com", safe=""
        ) + "&text=" + urllib.parse.quote(share_text, safe="")
        st.markdown(
            f'<a href="{line_url}" target="_blank" rel="noopener noreferrer">'
            '<button style="background:#06C755;color:white;border:none;padding:10px 20px;'
            'border-radius:6px;font-size:14px;cursor:pointer;">LINEでシェア</button></a>',
