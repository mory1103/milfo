# harel デザイン規約 — グリーンベース・グラスモーフィズム
...
- 深い森のグリーンを背景に、すりガラス（グラスモーフィズム）のカードが浮かぶ。
...
:root{
  ...
  --glass-shadow:0 8px 32px rgba(4,20,14,.35);
  --blur:16px;

  /* 既存変数の再定義 */
  --card:var(--glass-bg);
  --ink:#f2f7f4;                 /* 本文（ほぼ白、緑がかり） */
  --muted:rgba(242,247,244,.62); /* 補助テキスト（半透明白） */
  --accent:#34d399;              /* エメラルド（ボタン・メーター） */
  --accent-deep:#059669;         /* ボタングラデーションの下端 */
  --accent-soft:rgba(52,211,153,.16); /* 選択面・タグ背景 */
...
**禁止**: 変数を経由しない色の直書きを新たに増やさない。既存CSS内の
直書き色（`#fff`、`rgba(255,255,255,.85)`、`#b04a4a` 等）は上記トークン
または半透明白に置き換える。削除ボタンの赤だけは `#f8a5a5`（暗背景で
読める淡い赤）を使ってよい。
...
@supports not ((backdrop-filter:blur(1px)) or (-webkit-backdrop-filter:blur(1px))){
  .card,.acc-toggle{background:rgba(11,43,33,.88)}
}
```
...
`pointer-events:none`・`z-index:-1`。色はエメラルド系
（例 `rgba(52,211,153,.25)`、`rgba(110,231,183,.18)`、`rgba(16,185,129,.20)`）。
...
| `.btn-primary` | `linear-gradient(135deg,var(--accent),var(--accent-deep))`＋文字 `#06281c`（濃緑。白文字はエメラルド上でコントラスト不足）。hover で明度up＋`translateY(-1px)` |
| `.btn-ghost` | 透明面＋`--glass-border` 枠＋文字 `#a7f3d0` |
| メーター | track は `rgba(255,255,255,.15)`、fill は accent グラデーション |
| `.bar`（集計棒） | track `rgba(255,255,255,.12)`、fill は accent グラデーション |
| `.tag` | `--accent-soft` 面＋文字 `#a7f3d0`＋`1px solid rgba(52,211,153,.35)` |
| `.acc-toggle` | ガラスレシピ適用（既存の box-shadow 直書きを置換） |
| `#splash` | 背景グラデーションを緑に。ロゴ文字はそのまま白 |
| footer | `background:rgba(11,43,33,.80)`＋`backdrop-filter:blur(12px)`（下端もガラスに） |
| 削除ボタン | 文字色 `#f8a5a5` |
...
- [ ] `style.css` 内に旧配色（`#1b2a4a` `#7c6f9e` `#f6c9a8` `#4a6fa5` `#e8eef7` `#e3e6ee` `#6b7180`）が残っていない
